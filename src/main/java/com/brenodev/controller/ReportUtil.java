...
// Classe de geração de relatórios
	...
	public byte[] gerarRelatorio(List<?> listDados, String relatorio, ServletContext servletContext) throws Exception {
		...
		// Carrega o arquivo Japer passando os dados
		JasperPrint imprenssoraJasper = JasperFillManager.fillReport(caminhoJasper, new HashMap<>(), jrbcds);
		
		// Exporta para byte[] para fazer o download do PDF
		return JasperExportManager.exportReportToPdf(imprenssoraJasper);
	}
